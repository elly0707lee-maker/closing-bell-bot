...
async def call_claude(session: dict) -> str:
    ...
    for i, item in enumerate(items):
        t = item["type"]
        if t in ("user_text", "daeshin", "dokhagjushik", "teukjingju"):
            if t == "daeshin":
...
async def send_to_dashboard(result: str, date: str):
    """대시보드로 마감일지 전송"""
    ...
    try:
        async with httpx.AsyncClient(timeout=10) as client:
            await client.post(
                ...
                json={"content": result, "date": date},
            )
        logger.info("대시보드 마감일지 전송 완료")
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if any(kw in text for kw in ["정리해줘", "완성해줘", "출력해줘"]):
        ...
        try:
            ...
            # ✅ 대시보드로 자동 전송
            await send_to_dashboard(result, session["date"])
        except Exception as e:
