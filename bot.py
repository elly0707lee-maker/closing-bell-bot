...
- 다른 어떤 자료에서 수치를 가져오거나 추정하는 것 절대 금지
...
- 입력된 자료의 내용을 최대한 살릴 것. 자료가 많으면 출력도 그에 비례해서 풍부하게
...
if any(kw in text for kw in ["정리해줘", "완성해줘", "출력해줘"]):
    ...
    await update.message.reply_text(f"⏳ 마감일지 작성 중... (누적 자료 {count}건)")
    try:
...
try:
    ...
    await update.message.reply_text(f"⚠️ 사진 처리 실패\n오류: {e}")
```
