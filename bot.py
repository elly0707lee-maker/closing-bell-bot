...
변경:
- CLAUDE_MODEL: claude-sonnet-4-5 (deprecate됨) → claude-sonnet-4-6
- send_to_dashboard() 함수 추가 → 정리해줘 후 대시보드 자동 업데이트
- 환경변수 DASHBOARD_URL, API_SECRET 필요
"""
...
CLAUDE_MODEL = "claude-sonnet-4-6"  # ← 옛 4-5는 6/11쯤 retire됨
...
# ── 대시보드 자동 전송 ──────────────────────────────────
async def send_to_dashboard(content: str) -> tuple[bool, str]:
    """완성된 마감일지를 대시보드 /api/post/closing 으로 전송 (replace).

    반환: (성공 여부, 메시지)
    """
    if not DASHBOARD_URL:
        return False, "DASHBOARD_URL 환경변수 미설정"
    if not API_SECRET:
        return False, "API_SECRET 환경변수 미설정"
    try:
        async with httpx.AsyncClient(timeout=15) as client:
            ...
            if resp.status_code == 200:
                return True, f"OK ({len(content)}자)"
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    dashboard_status = "✅ 자동전송 ON" if (DASHBOARD_URL and API_SECRET) else "⚠️ 자동전송 OFF (환경변수 확인)"
    await update.message.reply_text(
        "📊 ClosingBell 봇 시작!\n\n"
        "• 자료 붙여넣기 → 누적 저장\n"
        "• 3/13 마감일지 생성 → 새 날짜 시작\n"
        "• 정리해줘 → 완성본 출력 + 대시보드 자동 업데이트\n"
        "• 사진/PDF 바로 전송 가능\n\n"
        f"📡 대시보드: {dashboard_status}\n"
        f"🤖 모델: {CLAUDE_MODEL}"
    )
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if any(kw in text for kw in ["정리해줘", "완성해줘", "출력해줘"]):
        ...
        try:
            ...
            # ── 대시보드 자동 전송 ─────────────────
            ok, msg = await send_to_dashboard(result)
            if ok:
                await update.message.reply_text(f"📤 대시보드 업데이트 완료 — {msg}")
            else:
                await update.message.reply_text(f"⚠️ 대시보드 전송 실패: {msg}")
        except Exception as e:
...
async def status(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    dashboard_status = "ON" if (DASHBOARD_URL and API_SECRET) else "OFF"
    await update.message.reply_text(
        f"📋 [{session['date']} 마감일지]\n"
        f"누적 자료: {len(session['items'])}건\n"
        f"종류: {', '.join(types) if types else '없음'}\n"
        f"📡 대시보드 자동전송: {dashboard_status}"
    )
...
def main():
    ...
    logger.info(f"ClosingBell 봇 시작 (모델: {CLAUDE_MODEL}, 대시보드: {DASHBOARD_URL or '미설정'})")
